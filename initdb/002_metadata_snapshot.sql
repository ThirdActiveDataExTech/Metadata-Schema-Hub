...
(
    snapshot_id        TEXT PRIMARY KEY,              -- 고유 식별자 (urn:{namespace}:metadata:{timestamp}-{hash12})
...
-- 컬럼 설명
COMMENT ON COLUMN metadata_snapshot.snapshot_id IS '고유 식별자: urn:{namespace}:metadata:{timestamp}-{hash12} (기본 namespace: wisenut)';
